...
async function run() {
  ...
  try {
    ...
    await fillField(page, 'date', new Date(Date.now() + 86_400_000).toISOString().slice(0, 10));
    await fillField(page, 'time', '10:30');
    await fillField(page, 'note', 'Confirmar medidas e alinhar próximo contato.');
    await saveDialog(page);
