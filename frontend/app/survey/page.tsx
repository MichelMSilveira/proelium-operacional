...
export default function SurveyPage() {
  ...
  useEffect(() => { apiGet<{ data?: Record<string, Item[]> }>('/api/data').then((payload) => setData({ surveys: payload.data?.surveys || [], surveyPoints: payload.data?.surveyPoints || [] })).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : 'Falha ao carregar levantamento.')); }, []);
