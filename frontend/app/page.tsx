...
export default function Page() {
  ...
  if (user) { const navigation = [['dashboard', 'Visão geral'], ['clients', 'Clientes'], ['projects', 'Projetos'], ['commercial', 'Comercial'], ['finance', 'Financeiro'], ['bi', 'Indicadores']]; return <main className="app-page"><aside><span className="eyebrow">PROELIUM</span><nav>{navigation.map(([key, label]) => <button className={view === key ? 'active' : ''} key={key} onClick={() => setView(key)}>{label}</button>)}</nav><button className="logout" onClick={async () => { await fetch('/api/auth/logout', { method: 'POST', credentials: 'include' }); setUser(null); }}>Sair</button></aside><section className="workspace"><header><span>{user.name || user.username}</span></header><section className="content"><p className="eyebrow">{navigation.find(([key]) => key === view)?.[1]}</p><h1>{view === 'dashboard' ? `Olá, ${user.name || user.username}` : 'Módulo em transição'}</h1><p>{view === 'dashboard' ? 'Shell autenticado em migração. Os módulos operacionais continuam disponíveis no aplicativo legado.' : 'Esta área será migrada em uma próxima fatia, com seus testes e permissões preservados.'}</p>{view === 'dashboard' && <><button onClick={async () => { const response = await fetch('/api/data', { credentials: 'include' }); setData(response.ok ? await response.json() : null); }}>Carregar resumo operacional</button>{data && <div className="stats">{['clients','projects','tasks','financialEntries'].map((key) => <article key={key}><strong>{data.data?.[key]?.length || 0}</strong><span>{key}</span></article>)}</div>}</>}</section></section><style jsx>{`.app-page{min-height:100vh;display:flex;background:#f5f1eb;color:#262828;font-family:Arial,sans-serif}aside{width:210px;display:flex;flex-direction:column;gap:28px;padding:28px 16px;background:#3b4a3a;color:#fff}nav{display:grid;gap:6px}nav button,.logout{padding:11px 12px;border:0;border-radius:7px;text-align:left;background:transparent;color:#fff;cursor:pointer}nav button.active,nav button:hover{background:#cb6445}.logout{margin-top:auto;background:#ffffff1c}.workspace{flex:1}.workspace header{padding:18px 6%;text-align:right;background:#fff;border-bottom:1px solid #ded6cb}.content{max-width:1000px;margin:auto;padding:48px 6%}.content h1{font:500 34px Georgia,serif}.content p{color:#66705f}.content>button{border:0;border-radius:7px;padding:10px 14px;background:#cb6445;color:#fff;font-weight:700;cursor:pointer}.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-top:28px}.stats article{display:grid;gap:6px;padding:20px;border-radius:12px;background:#fff;box-shadow:0 5px 20px #26282812}.stats strong{font-size:28px;color:#3b4a3a}.stats span{font-size:12px;color:#66705f}@media(max-width:650px){aside{width:145px;padding:20px 9px}.stats{grid-template-columns:repeat(2,1fr)}}`}</style></main>; }
  return (
